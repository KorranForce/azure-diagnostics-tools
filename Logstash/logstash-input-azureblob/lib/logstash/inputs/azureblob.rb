...
class LogStash::Inputs::LogstashInputAzureblob < LogStash::Inputs::Base
	...
	def create_registry(blob_items)
		...
		blob_items.each do |blob_item|
			initial_offset = registry_create_policy == 'resume' ? blob_item.properties[:content_length] : 0
			registry_hash[blob_item.name] = LogStash::Inputs::RegistryItem.new(blob_item.name, blob_item.properties[:etag], nil, initial_offset, 0)
		end # each
		save_registry(registry_hash, lease)
		@azure_blob.release_blob_lease(container, registry_path, lease)
		registry_hash
	...
	def load_registry
		...
		deserialize_registry_hash(registry_blob_body)
